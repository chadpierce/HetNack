...
const struct Role roles[NUM_ROLES+1] = {
    { { "Priest", 0 },
      ...
      MH_HUMAN | MH_ELF | ROLE_MALE | ROLE_LAWFUL | ROLE_NEUTRAL
          | ROLE_CHAOTIC,
    ...
    { { "Rogue", 0 },
      ...
      MH_HUMAN | MH_ORC | ROLE_MALE | ROLE_CHAOTIC,
      /* Str Int Wis Dex Con Cha */
      { 7, 7, 7, 10, 7, 6 },
      { 20, 10, 10, 30, 20, 10 },
      /* Init   Lower  Higher */
    ...
    { { "Ranger", 0 },
      ...
      MH_HUMAN | MH_ELF | MH_GNOME | MH_ORC | ROLE_MALE
    ...
    { { "Fighter", 0 },
      ...
      S_GIANT,
      ART_ORB_OF_FATE,
      MH_HUMAN | MH_DWARF | ROLE_MALE | ROLE_LAWFUL | ROLE_NEUTRAL,
      /* Str Int Wis Dex Con Cha */
      { 10, 7, 7, 7, 10, 7 },
      { 30, 6, 7, 20, 30, 7 },
      /* Init   Lower  Higher */
...
const struct Race races[NUM_RACES + 1] = {
    ...
    },
    {
        ...
        PM_DWARF_MUMMY,
        PM_DWARF_ZOMBIE,
        MH_DWARF | ROLE_MALE | ROLE_LAWFUL,
        MH_DWARF,
        MH_DWARF | MH_GNOME,
        MH_ORC,
        /*    Str     Int Wis Dex Con Cha */
    ...
    },
    {
        ...
        PM_GNOME_MUMMY,
        PM_GNOME_ZOMBIE,
        MH_GNOME | ROLE_MALE | ROLE_NEUTRAL,
        MH_GNOME,
        MH_DWARF | MH_GNOME,
        MH_HUMAN,
        /*  Str    Int Wis Dex Con Cha */
    ...
    int rolenum, int racenum, int gendnum, int alignnum)
{
    ...
    *suppliedbuf = '\0';
    ...
    /* HetNack: gender removed; never append gender adj or list "gender"
       as a post-attribute to be picked. */
    (void) gendercount;
    (void) gendnum;
    /* <your lawful> */
...
plsel_startmenu(int ttyrows, int aspect)
{
    ...
    if (!svp.plname[0] || ROLE < 0 || RACE < 0 || GEND < 0 || ALGN < 0) {
        /* "<role> <race.noun> <alignment>" */
        Sprintf(qbuf, "%.20s %.20s %.20s",
                rolename,
                (RACE < 0) ? "<race>" : races[RACE].noun,
                (ALGN < 0) ? "<alignment>" : aligns[ALGN].adj);
    } else {
        /* "<name> the <alignment> <race.adjective> <role>" */
        Sprintf(qbuf, "%.20s the %.20s %.20s %.20s",
                svp.plname,
                aligns[ALGN].adj,
                races[RACE].adj,
                rolename);
